...
**项目名**：零活工人工资管理 App  
**当前阶段**：📍 **需求冻结为 V1.3（三实体模型），新 M2.0 编码中**

**现行需求基线**：`docs/requirements_V1.3.txt`（2026-08-09 决策版，三实体：Worker + WageRecord + Worksite）
**历史需求存档**：
- `docs/requirements_V1.2.txt`（工人模型，未引入 Worksite，仅供 diff 参考）
...
- 同名强制改名：同名字符串全局唯一，UI 强制妈妈改名为张姐A / 张姐(早班)
...
📍 当前：开干新 M2.0（V1.3 三实体首刀：Schema 迁移 1→2 + Worksite CRUD + 首页聚合 + 批量添加 + 工人选择器 + 长按详情 + 同名强制改名）

🚧 接下来要做（V1.3 切片，按优先级）：
- [ ] 1. 新 M2.0（本次）：三实体 CRUD + 首页聚合 + 批量添加 + 同名强制改名（3-3.5 天）
- [ ] 2. 新 M3：ML Kit 扫码 + 图片存储（2-3 天）
- [ ] 3. 新 M4：历史日期 + 工人筛选 + 订单汇总（1-2 天）
- [ ] 4. 新 M5：CSV 导出 + 清空工资（1-2 天）
```
...
- 需求文档（V1.3 决策版）：`docs/requirements_V1.3.txt`
- 需求文档（V1.2 存档）：`docs/requirements_V1.2.txt`（工人模型，未引入 Worksite）

...
**项目名**：零活工人工资管理 App  
**当前阶段**：📍 **需求冻结为 V1.2，正式开发中**

**现行需求基线**：`docs/requirements_V1.2.txt`（2026-08-09 决策版，三层视图：工人列表 → 工人详情 → 账单编辑）
**历史需求存档**：
- `docs/requirements_V1.1.txt`（流水模型，已被 V1.2 取代，仅供 diff 参考）
- `docs/requirements_V1.0.txt`（初版，仅供 diff 参考）

**一句话定位**：帮助带班妈妈（保洁零活）按"工人 → 账单"层级管理每日出工工资，扫码或手动添加账单，逐张标记支付状态，杜绝漏付、错付。

**产品模型**（V1.2 关键变更）：
- **顶层是工人**，不是"今天的流水"
- 一个人下面有 N 张账单（任意时间）
- 支付最小单元是"账单"（不是人）
- 三层视图：① 工人列表（首页）② 工人详情（账单列表）③ 账单编辑（M2）
...
- [x] **V1 决策版需求（流水模型）M1 + M2.1 简化同名交互**（已 commit，准备推翻重做）
- [x] V1.2 决策版需求（工人模型，三层视图）落地到 docs/requirements_V1.2.txt
- [x] CLAUDE.md 升级到 v2.2

📍 当前：准备开始新 M1（V1.2 工人模型的第一刀）

🚧 接下来要做（V1.2 切片，按优先级）：
- [ ] 1. 新 M1：工人列表首页 + 工人详情页 + 添加账单（3-4 天）
- [ ] 2. 新 M2：标记已付 / 撤销 / 删除 / 编辑账单（2 天）
- [ ] 3. 新 M3：备注/地点字段（schema 迁移 1→2）+ ML Kit 扫码 + 图片存储（2-3 天）
- [ ] 4. 新 M4：历史日期 + 工人筛选（1-2 天）
- [ ] 5. 新 M5：CSV 导出 + 清空工资（1-2 天）
```
...
- 需求文档（V1.2 决策版）：`docs/requirements_V1.2.txt`
- 需求文档（V1.1 存档）：`docs/requirements_V1.1.txt`（流水模型，已被 V1.2 取代）

...
**一句话定位**：**记账工具**——帮助带班妈妈（保洁零活）按"角色（工人）→ 订单 → 工区"三层实体清晰记录每日出工工资。核心是"记明白账"，自动支付 / 扫码识别 / 云备份都是辅助功能。

**产品模型**（V1.3 决策版 + 2026-08-24 记账工具定位）：
...
- ✅ M3.1：DatePicker Bug 修复 + 大升级（BOM 2026.01.01 / Kotlin 2.2.20 / Room 2.7.2 / K2 plugin）
- ✅ M4：订单 Tab 实现 + BillCard 重写 + BottomSheet 滚动修复
- ✅ M4.5：自动备份到私有目录（BackupManager）
- ⏳ M4.1-M4.4：月度切换 / 累计统计 / 工人排行 / 账单搜索（记账核心）
- ⏳ M5：清空工资（月底结清）
- ⏸️ M7+：扫码 / CSV / 云备份（记账非核心，远期）
...
📍 当前：M4.5 完成（记账工具核心：记账 / 汇总 / 筛选 已基本就绪）

🚧 接下来要做（**记账工具**定位：把记账写明白）：

### P0 - 立即（核心记账体验）
- [ ] M4.1: WorkerListScreen 月度切换 + 当月汇总（1 天）
  - 顶部加"上一月 / 本月 / 下一月"切换器
  - 月度汇总卡：未付总额 / 已付总额 / 总笔数
  - "← 当前 →" 改为 "← 月份 →"
- [ ] M4.2: WorkerListScreen 累计统计卡（0.5 天）
  - "妈妈使用至今"的总工资 / 总笔数 / 工人数

### P1 - 近期（辅助记账）
- [ ] M4.3: 工人排行（按工资金额 / 笔数排序，1 天）
- [ ] M4.4: 账单搜索（按工人名 / 备注 / 工区名，1 天）
- [ ] M5: 清空工资功能（按月 / 按工人，1 天）
  - 月底结清"已发"账单（标记 + 可选删除）

### P2 - 中期（便捷性）
- [ ] M6: 快速添加优化（默认填上次工区 / 备注，0.5 天）

### P3 - 远期 / 代办（记账非核心）
- [ ] M7: 扫单小票二维码录入（记账辅助，**优先级低**，1-2 天）
- [ ] M8: CSV 导出到 Downloads（**代办**，触发条件：换手机 / 卸载丢数据）
- [ ] M9: CSV 导入 + 启动检测恢复（**代办**，跟 M8 一起做）
- [ ] M10: 云备份（Google Drive OAuth，**可选**，超出 MVP）

📋 优先级重排决策（M4.5 讨论 2026-08-24）：

**产品定位**：**记账工具**（不是支付自动化）

**为什么把扫码 / 自动支付 / 云备份推后**：
1. 妈妈的使用场景是"记工资账"——不是扫码识别单据、不是自动支付
3. "自动支付可以人自己去"——标记已付是手动操作
3. 扫码不是核心（M7）——手工记账完全够用
4. 云备份 / CSV 导入是换手机场景才需要——手机短期不变，暂缓

**当前已足够好的**：记账核心功能（添加 / 查看 / 筛选 / 按日期查询）已就绪

📋 CSV 持久化代办决策（M4.5 讨论 2026-08-24）：
- M4.5 已实现 A 自动备份（filesDir/backup/wage_manager.db）
- M8/M9 CSV 导出 + 导入推迟，触发条件：
  - 妈妈换新手机
  - 真发生卸载丢数据
  - 妈妈主动要求"想要换手机也能恢复"

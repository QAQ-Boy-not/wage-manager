...
**项目名**：零活工人工资管理 App  
**当前阶段**：📍 **V1.3 决策版 + M2.1 反馈修复已完成，CI 跑通中**

**现行需求基线**：`docs/requirements_V1.3.txt`（V1.3 决策版 + M2.1 增量更新）
**历史需求存档**：
...
**M2.1 关键修复**（相对 V1.3 决策版）：
- 新建工区 / 新建工人 入口（避免"批量添加"卡住）
- 账单详情 Tab 切换（去掉上下分组）
- "全部标记已付"从首页移到详情页（操作不放在概览层级）
- 详情页 +/- 走后端 addBillToWorker 路径（不走 registerBill 同名检查）
- 编辑/新增成功后自动关闭 BottomSheet
...
🚧 接下来要做（V1.3 切片，按优先级）：
- [ ] 1. 新 M3：ML Kit 扫码 + 图片存储（2-3 天）
- [ ] 2. 新 M4：历史日期 + 工人筛选 + 汇总（1-2 天）
- [ ] 3. 新 M5：CSV 导出 + 清空工资（1-2 天）
```
...
- 需求文档（V1.3 + M2.1 决策版）：`docs/requirements_V1.3.txt`
